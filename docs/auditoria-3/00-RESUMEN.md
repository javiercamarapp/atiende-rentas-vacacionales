...
| A3-NOTIF-01 | Medio | Notificaciones | **CORREGIDO** — Firma HMAC de webhook saliente sin componente de tiempo (sin anti-replay estructural, a diferencia de Stripe) | `packages/domain/test/notificaciones/webhookFirma.test.ts` + `apps/api/test/notificaciones/webhookSaliente.test.ts` (PASAN, con prueba real de replay) |
