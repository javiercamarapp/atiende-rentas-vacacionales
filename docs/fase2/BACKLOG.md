...
| H-091 | Modo degradado de solo-lectura del calendario si la BD primaria no responde | REQ-166 | §Operación-5 (corregido — §Operación-3 describe backup, no este criterio) | MUST | L | hecho (cierre H-091, rama `closure/h091-modo-degradado-calendario`) — las dos piezas que quedaban pendientes desde Lote 3.0 (`cc45be9`) ya están: (a) **wiring real**: `GET /unidades/:id/calendario` (`apps/api/src/routes/unidades.ts`) ahora enruta su lectura a través de `EnrutadorLecturaReplica` real (`apps/api/src/db/contexto.ts:poolConsultableParaLectura` + `apps/api/src/app.ts` construye `poolReplica` desde `DATABASE_URL_REPLICA` opcional, exactamente el mismo criterio de "ausencia nunca cambia comportamiento" que `DATABASE_URL`); (b) **modo degradado**: `GET /health/detallado` (`apps/api/src/workers/observabilidad/rutas.ts`) detecta que el primario no responde (la misma sonda real que ya hacía, sin una nueva) y expone `modoDegradadoCalendario`/`pushAutomaticoHabilitado`, apagando automáticamente `sync.push_automatico` (idempotente, nunca se reactiva solo); esa pausa es efectiva de verdad sobre el push real — `procesarPendientesOutbox` (`outboxWorker.ts`) y el cron `GET /internal/cron/outbox-worker` (`cronOutboxWorker.ts`) respetan el flag y no procesan nada mientras esté apagado (antes de este cierre el flag `sync.push_automatico` se seteaba pero NINGÚN camino de código lo leía — una pausa decorativa, ahora corregida); (c) **UI**: `apps/web/src/pages/calendario/components/ModoDegradadoBanner.tsx`, mostrado en `CalendarioMaestroPage.tsx` mientras `GET /health/detallado` reporte el modo degradado. Probado con DOS clusters `embedded-postgres` reales (mismo límite honesto que antes: nunca streaming replication real) — apagar el PROCESO del primario de verdad (`apps/api/test/integration/calendarioModoDegradadoPrimario.test.ts`) y apagar el de la réplica de verdad (`apps/api/test/integration/calendarioReplicaLectura.test.ts`) — más pruebas unitarias del gate de push (`outboxWorker.test.ts`, `cronOutboxWorker.test.ts`, `rutas.test.ts`). Gates: `npm run typecheck`/`lint` en verde en los 7 workspaces; `npm run test`/`test:integration` en verde (ver evidencia de esta sesión) |
